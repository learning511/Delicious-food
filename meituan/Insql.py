...
def shop_db(info_dict):
    ...
    # 判断是否已存在记录
    info = SQLsession.query(table_info).filter_by(shop_id=temp_id).first()
    if info:
...
def comment_db(info_dict):
    ...
    # 判断是否已存在记录
    info = SQLsession.query(table_info1).filter_by(reviewId=temp_id).first()
    if info:
        info.shop_id = info_dict.get('shop_id','')
